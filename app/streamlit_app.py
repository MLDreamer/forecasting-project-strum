"""Fontana Candle — Forecasting Dashboard

Entry point.  Loads the 8 contract tables once (cached on manifest hash),
stores them in session_state, and delegates to the four pages.
...
# ── Load data (once per manifest hash) ──────────────────────────────────────
if not has_data():
    st.warning(
        "No forecast data found in `outputs/latest/`.  "
        "Run the pipeline first:\n\n"
        "```bash\npython -m forecasting.run --config configs/fontana.yaml\n```",
        icon="⚠️",
    )
    st.stop()
